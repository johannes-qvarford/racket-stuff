...
(define (fibs-stream)
    (define (fibs-rec a b)
        (stream-cons (+ a b) (fibs-rec b (+ a b)))
    )
    (fibs-rec 0 1)
)

(define (stream-nth n s)
    (if (eq? n 1) (stream-first s)
        (stream-nth (- n 1) (stream-rest s))))

(ce (stream-first (fibs-stream)) 1)
...
(ce (stream->list (stream-take-until (lambda (x) (> x 10)) (fibs-stream)) ) '(1 2 3 5 8))
