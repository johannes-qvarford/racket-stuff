...
(define (smallest-factor n)
        (stream-find (curry no-remainder? n)
                     (stream-tail (positive-natural-numbers-stream) 2)))
...
(define (smallest-factors n)
        (let ((f (smallest-factor n)))
             (if (= f n) (list n) (cons f (smallest-factors (/ n f))))))
...
(define (make-bag-entry item count) (list item count))

(define (bag-entry-item b) (car b))

(define (bag-entry-count b) (cadr b))

(define (empty?-or-first-not= a b) (or (empty? a) (not= b (car a))))
...
(define (ordered-list->bag xs)
        (define (iter item items-left item-count)
                (cond [(empty?-or-first-not= items-left item)
                       (cons (make-bag-entry item item-count) (ordered-list->bag items-left))]
                      [else (iter item (cdr items-left) (plus1 item-count))]))
        (if (empty? xs) '() (iter (car xs) xs 0)))
...
(define (max-bag-entry a b)
        (if (> (bag-entry-count a) (bag-entry-count b)) a b))

(define (union-bags-by-max-count a b)
        (define (union-non-empty-bags-by-max-count a b)
                (let* ((ae (car a)) (aei (bag-entry-item ae))
                                    (aec (bag-entry-count ae))
                                    (be (car b))
                                    (bei (bag-entry-item be))
                                    (bec (bag-entry-count be)))
                      (cond [(> aei bei) (union-non-empty-bags-by-max-count b a)]
                            [(= aei bei)
                             (cons (if (> aec bec) ae be) (union-bags-by-max-count (cdr a) (cdr b)))]
...
(define one-to-twenty
        (stream-take (stream-tail (positive-natural-numbers-stream) 1) 20))
