...
(define (stream-contains-any? f xs)
    (not (stream-empty? (stream-filter f xs))))

(define (divisible-by-2-3-digit-numbers? x)
    (stream-contains-any? (lambda (n) (qoutient-is-3-digit-number? x n)) (in-range 100 1000)))
...
(define divs (stream-filter divisible-by-2-3-digit-numbers? palindromes-less-than-1mil))
