...
(define (positive-natural-numbers-stream)
        (define (recur n) (stream-cons n (recur (plus1 n))))
        (recur 0))
...
(ce (stream->list
     (stream-take-until (lambda (x) (> x 5)) (positive-natural-numbers-stream)))
    '(0 1 2 3 4 5))
...
(define (in-range-list a b) (stream->list (in-range a b)))

(define (find p xs)
        (cond [(empty? xs) '()] [(p (car xs)) (car xs)] [else (find p (cdr xs))]))
